...
Given(/^user (.*) who is an organizer of (.*) should be able to delete it$/) do |username, eventname|
...
Given(/^an event (.*) that has started$/) do |eventname|
  create_event_named(eventname, startdate: Date.yesterday)
end

Given(/^user (.*) who is an organizer of (.*) should not be able to delete it$/) do |username, eventname|
  user = create_user_named(username)
  event = event_named(eventname)
  event.organizers << Organizer.new(user: user)
